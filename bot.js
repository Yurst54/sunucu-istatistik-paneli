...
client.on("guildMemberAdd", async(member) => { //Sunucuya biri girdiğinde panelde ki her şey değişir
...
client.on("guildMemberRemove", async(member) => { //Sunucuya biri girdiğinde panelde ki her şey değişir
  let sunucupaneli = await db.fetch(`sunucupanel_${member.guild.id}`)
  if(sunucupaneli) {
    let rekoronline = await db.fetch(`panelrekor_${member.guild.id}`)
    let toplamuye = member.guild.channels.find(x =>x.name.startsWith("Toplam Üye •"))
